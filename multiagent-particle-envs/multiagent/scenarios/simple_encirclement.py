...
class Scenario(BaseScenario):
    ...
    def make_world(self):
        ...
        for i, agent in enumerate(world.agents):
            ...
            if not agent.adversary:
                self.group1.append(agent)
        world.agents[0].color = np.array([255,0,0])/255
        world.agents[1].color = np.array([0, 255, 0])/255
        world.agents[2].color = np.array([0,0,255])/255
        # add landmarks
    ...
    def getSpacing_rew(self,agent, world):
        ...
        myIndex = self.group1.index(agent) # I am the agent NO.i in angle list
        plusIndex = 0 if myIndex==len(self.group1)-1 else myIndex+1
        miusIndex = len(self.group1)-1 if myIndex==0 else myIndex-1
        # print('agent: ',self.group1[miusIndex].name,self.group1[myIndex].name,self.group1[plusIndex].name)
        myPos = self.group1[myIndex].state.p_pos - self.group1[myIndex].goal_a.state.p_pos
        plusPos = self.group1[plusIndex].state.p_pos - self.group1[plusIndex].goal_a.state.p_pos
        miusPos = self.group1[miusIndex].state.p_pos - self.group1[miusIndex].goal_a.state.p_pos
